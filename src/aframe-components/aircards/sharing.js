/*
Description: This component enables sharing of custom title, text, url and visual files through the user's native device functionality.
If the screenshotMode is toggled true, then the component will screenshot the a-scene by default, or the entire screen if hideUI is additionally toggled false.

Watermark functionality: Separate files with commas. Positions, widths and heights are in order according to the watermarks array
*/
...
export const Sharing = {
  ...
  init () {
    ...
    for (let i = 0; i < watermarks.length; i++) {
      ...
      try {
        watermarkImageSrc = `/sharing/${watermarks[i]}`
      } catch (e) {
    ...
    const canShareTogether = () => {
      ...
      const errorText = ''
      if (os === 'iOS') {
        return parseInt(osVersion.substring(0, 2)) >= 16
        // Register for non-iOS devices
      } else {
        // not iOS device, share both image and text
        return true
      }
    }
      ...
      try {
        ...
        if (this.data.files.length > 0) {
          ...
          this.data.files.forEach(currFile => {
            ...
            fetch(`/sharing/${folder}${currFile}`).then(function (response) {
        ...
        navigator.clipboard.writeText(`${ShareData.title} ${ShareData.text} ${ShareData.url}`).then(() => {
          if (this.data.files.length > 0) {
            this.data.files.forEach(currFile => {
              ...
              a.href = require(`/sharing/${currFile}`)
